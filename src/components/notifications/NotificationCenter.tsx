...
// Zelfde limiet als de volledige /notificaties-pagina — op mobiel wordt dit
// nu letterlijk diezelfde ervaring, direct vanaf de belknop.
const MOBILE_NOTIFICATION_LIMIT = 50;
...
export function NotificationCenter({ count, variant }: { count: number; variant: "sidebar" | "mobile" }) {
  ...
  function handleOpen() {
    ...
    if (willOpen && notifications === null) {
      setLoading(true);
      void getRecentNotificationsAction(variant === "mobile" ? MOBILE_NOTIFICATION_LIMIT : undefined)
        .then((result) => {
